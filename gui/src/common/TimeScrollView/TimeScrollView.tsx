...
import { TickSet } from 'common/TimeScrollView/YAxisTicks';
import TimeWidgetToolbarEntries, { DefaultToolbarWidth } from 'common/TimeWidgetToolbarEntries';
import { Divider, ToolbarItem } from 'common/Toolbars';
...
import { TimeseriesLayoutOpts } from 'MultiTrialRaster/MultiTrialRaster';
...
// This might be useful functionality elsewhere, but here it's just kind of a hack
export const useMultiplePanelDimensions = (width: number, height: number, panelCounts: number[], panelSpacings: number[], margins?: Margins) => {
    return useMemo(() => {
        const definedMargins = margins ?? defaultMargins
        const panelWidth = width - definedMargins.left - definedMargins.right
        const heightExMargin = height - definedMargins.top - definedMargins.bottom
        const panelHeights = panelCounts.map((count, i) => {
            return ((heightExMargin - (panelSpacings[i] * (count - 1))) / count)
        })
        panelHeights.forEach((h, i) => {
            if (h < 1) {
                console.warn(`Panel height ${i} is sub-1-pixel (${h}); cannot fit requested number of panels ${panelCounts[i]} in drawing space.`)
            }
        })
        
        return {panelWidth, panelHeights}
    }, [margins, width, height, panelCounts, panelSpacings])
}
...
const TimeScrollView = <T extends {[key: string]: any}> (props: TimeScrollViewProps<T>) => {
    ...
    const {panelHeights: [panelHeight], panelWidth} = useMultiplePanelDimensions(width - toolbarWidth, height, [panels.length], [panelSpacing], definedMargins)
